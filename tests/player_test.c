...
int main(int argc, char **argv)
{
    ...
    if (all || test == 36)
        test2_player_print();
    if (all || test == 37)
        test1_player_set_max_health();
    if (all || test == 38)
        test2_player_set_max_health();
    if (all || test == 39)
        test1_player_get_max_health();
    if (all || test == 40)
        test2_player_get_max_health();
    if (all || test == 41)
        test1_player_set_damage();
    if (all || test == 42)
        test2_player_set_damage();
    if (all || test == 43)
        test1_player_get_damage();
    if (all || test == 44)
        test2_player_get_damage();
    if (all || test == 45)
        test1_player_set_dtaken();
    if (all || test == 46)
        test2_player_set_dtaken();
    if (all || test == 47)
        test1_player_get_dtaken();
    if (all || test == 48)
        test2_player_get_dtaken();
    if (all || test == 49)
        test1_player_set_n_turn();
    if (all || test == 50)
        test2_player_set_n_turn();
    if (all || test == 51)
        test1_player_get_n_turn();
    if (all || test == 52)
        test2_player_get_n_turn();
    if (all || test == 53)
        test1_player_set_max_turn();
    if (all || test == 54)
        test2_player_set_max_turn();
    if (all || test == 55)
        test1_player_get_max_turn();
    if (all || test == 56)
        test2_player_get_max_turn();
...
{
    Player *p = player_create(1);
    PRINT_TEST_RESULT(player_set_max_health(p, 100) == OK);
    player_destroy(p);
}
...
{
    Player *p = NULL;
    PRINT_TEST_RESULT(player_set_max_health(p, 100) == ERROR);
}

void test1_player_get_max_health()
{
    Player *p = player_create(1);
    player_set_max_health(p, 150);
    PRINT_TEST_RESULT(player_get_max_health(p) == 150);
    player_destroy(p);
}
...
{
    Player *p = player_create(1);
    PRINT_TEST_RESULT(player_set_damage(p, 25) == OK);
    player_destroy(p);
}
...
{
    Player *p = NULL;
    PRINT_TEST_RESULT(player_set_damage(p, 25) == ERROR);
}

void test1_player_get_damage()
{
    Player *p = player_create(1);
    player_set_damage(p, 30);
    PRINT_TEST_RESULT(player_get_damage(p) == 30);
    player_destroy(p);
}
...
{
    Player *p = player_create(1);
    PRINT_TEST_RESULT(player_set_dtaken(p, 10) == OK);
    player_destroy(p);
}
...
{
    Player *p = NULL;
    PRINT_TEST_RESULT(player_set_dtaken(p, 10) == ERROR);
}

void test1_player_get_dtaken()
{
    Player *p = player_create(1);
    player_set_dtaken(p, 12);
    PRINT_TEST_RESULT(player_get_dtaken(p) == 12);
    player_destroy(p);
}
...
{
    Player *p = player_create(1);
    PRINT_TEST_RESULT(player_set_n_turn(p, 3) == OK);
    player_destroy(p);
}
...
{
    Player *p = NULL;
    PRINT_TEST_RESULT(player_set_n_turn(p, 3) == ERROR);
}

void test1_player_get_n_turn()
{
    Player *p = player_create(1);
    player_set_n_turn(p, 5);
    PRINT_TEST_RESULT(player_get_n_turn(p) == 5);
    player_destroy(p);
}
...
{
    Player *p = player_create(1);
    PRINT_TEST_RESULT(player_set_max_turn(p, 10) == OK);
    player_destroy(p);
}
...
{
    Player *p = NULL;
    PRINT_TEST_RESULT(player_set_max_turn(p, 10) == ERROR);
}

void test1_player_get_max_turn()
{
    Player *p = player_create(1);
    player_set_max_turn(p, 12);
    PRINT_TEST_RESULT(player_get_max_turn(p) == 12);
    player_destroy(p);
}
