...
int main(int argc, char **argv)
{
    ...
    if (all || test == 39)
        test1_character_set_follow();
    if (all || test == 40)
        test2_character_set_follow();
    if (all || test == 41)
        test1_character_get_follow();
    if (all || test == 42)
        test2_character_get_follow();
    if (all || test == 43)
        test1_character_set_damage();
    if (all || test == 44)
        test2_character_set_damage();
    if (all || test == 45)
        test1_character_get_damage();
    if (all || test == 46)
        test2_character_get_damage();
...
void test1_character_set_follow()
{
    Character *c = character_create(1);
    PRINT_TEST_RESULT(character_set_following(c, 10) == OK);
    character_destroy(c);
}

void test2_character_set_follow()
{
    Character *c = NULL;
    PRINT_TEST_RESULT(character_set_following(c, 10) == ERROR);
}

void test1_character_get_follow()
{
    Character *c = character_create(1);
    character_set_follow(c, 20);
    PRINT_TEST_RESULT(character_get_following(c) == 20);
    character_destroy(c);
}
...
{
    Character *c = character_create(1);
    PRINT_TEST_RESULT(character_set_damage(c, 15) == OK);
    character_destroy(c);
}
...
{
    Character *c = NULL;
    PRINT_TEST_RESULT(character_set_damage(c, 15) == ERROR);
}

void test1_character_get_damage()
{
    Character *c = character_create(1);
    character_set_damage(c, 25);
    PRINT_TEST_RESULT(character_get_damage(c) == 25);
    character_destroy(c);
}
...
{
    Character *c = NULL;
    PRINT_TEST_RESULT(character_get_damage(c) == -1);
}