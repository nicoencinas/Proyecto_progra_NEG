...
$(BIN_DIR)/$(TARGET): $(OBJ_DIR)/command.o $(OBJ_DIR)/game_loop.o $(OBJ_DIR)/game.o \
                      $(OBJ_DIR)/graphic_engine.o $(OBJ_DIR)/space.o $(OBJ_DIR)/game_management.o \
                      $(OBJ_DIR)/object.o $(OBJ_DIR)/player.o $(OBJ_DIR)/game_actions.o \
                      $(OBJ_DIR)/set.o $(OBJ_DIR)/character.o $(OBJ_DIR)/link.o \
                      $(OBJ_DIR)/inventory.o  $(OBJ_DIR)/use.o $(OBJ_DIR)/game_rules.o
	$(CC) -o $@ $^ $(LDFLAGS)
...
$(OBJ_DIR)/use.o: $(SRC_DIR)/use.c $(INC_DIR)/use.h 
	$(CC) $(CFLAGS) -c $< -o $@

$(OBJ_DIR)/game_rules.o: $(SRC_DIR)/game_rules.c $(INC_DIR)/game_rules.h 
	$(CC) $(CFLAGS) -c $< -o $@

$(OBJ_DIR)/game_loop.o: $(SRC_DIR)/game_loop.c $(INC_DIR)/graphic_engine.h $(INC_DIR)/game.h $(INC_DIR)/command.h 
	$(CC) $(CFLAGS) -c $< -o $@

$(OBJ_DIR)/game.o: $(SRC_DIR)/game.c $(INC_DIR)/game.h $(INC_DIR)/game_management.h 
	$(CC) $(CFLAGS) -c $< -o $@
...
$(OBJ_DIR)/game_management.o: $(SRC_DIR)/game_management.c $(INC_DIR)/game_management.h $(INC_DIR)/character.h 
	$(CC) $(CFLAGS) -c $< -o $@
