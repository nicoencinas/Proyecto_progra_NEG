...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game)
{
  ...
  if (last_cmd == CHAT || last_cmd == ATTACK || last_cmd == INSPECT || last_cmd == MOVE || last_cmd == RECRUIT || last_cmd == ABANDON || last_cmd == USE || last_cmd == LOAD || last_cmd == SAVE || last_cmd == COLAB)
  {
    msg = game_get_s(game);
    if (msg != NULL && msg[0] != '\0')
    {
