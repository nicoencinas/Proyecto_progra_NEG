...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game)
{
  ...
  Space *space_act = NULL, *space_back = NULL, *space_next = NULL, *space_left = NULL, *space_right = NULL, *space_up = NULL, *space_down = NULL;
  char str[255];
  ...
  if ((id_act = game_get_player_location(game)) != NO_ID)
  {
    ...
    space_up = game_get_space(game, id_up);
    space_down = game_get_space(game, id_down);
    ...
    for (i = 0; i < n_chars; i++)
    {
      ...
      if (temp_char && character_get_health(temp_char) > 0)
      {
        Id char_loc = character_get_location(temp_char);

        if (char_loc == WITH_PLAYER)
        {
          sprintf(r_icon, "%.5s", character_get_gdesc(temp_char));
        }
        else if (char_loc == id_act)
        {
          sprintf(e_icon, "%.5s", character_get_gdesc(temp_char));
        }
      }
    }

    sprintf(p_c_icon, "%s %s %s", p_icon, r_icon, e_icon);
    ...
    if (id_up != NO_ID && space_up != NULL)
    {
      ...
      for (i = 0; i < 5; i++)
      {
        sprintf(str, "                                      | %-13.13s |", space_get_gdesc_at(space_up, i));
        screen_area_puts(ge->map, str);
      }
      sprintf(str, "                                      |               |");
      screen_area_puts(ge->map, str);
    ...
    for (i = 0; i < 5; i++)
    {
      ...
      {
        sprintf(str, (i == 2) ? "           | %-13.13s |         <| %-13.13s |>             | %-13.13s |" : "           | %-13.13s |          | %-13.13s |             | %-13.13s |",
      ...
      {
        sprintf(str, (i == 2) ? "           | %-13.13s |         <| %-13.13s |" : "           | %-13.13s |          | %-13.13s |",
    ...
    /* DOWN */
    if (id_down != NO_ID && space_down != NULL)
    {
      sprintf(str, "                                      +---------------+ ");
      screen_area_puts(ge->map, str);
      sprintf(str, "                                      |               |");
      ...
      for (i = 0; i < 5; i++)
      {
        sprintf(str, "                                      | %-13.13s |", space_get_gdesc_at(space_down, i));
        screen_area_puts(ge->map, str);
      }
      sprintf(str, "                                      |               |");
      screen_area_puts(ge->map, str);
  ...
  screen_area_puts(ge->descript, "   Objects in this area:");
  /*Solo los de el act*/
  ...
  if (space_act != NULL)
  {
    for (i = 0; i < game_get_n_obj(game); i++)
    {
      ...
      if (temp_obj != NULL)
      {
        obj_id = object_get_id(temp_obj);
        if (space_has_object(space_act, obj_id))
        {
  ...
  screen_area_puts(ge->descript, "   Characters here:");
  n_chars = game_get_n_characters(game);
  for (i = 0; i < n_chars; i++)
  {
    ...
    if (ch != NULL)
    {
      /* Si el ch está en la sala del player o si lo sigue */
      if (character_get_location(ch) == player_loc || character_get_following(ch) == player_get_id(game_get_player(game)))
      {
        sprintf(str, "      %s %s (HP: %d)",
  ...
  sprintf(str, "   Player %d: space %ld  HP: %d/%d  ATK: %d",
          game_get_turn(game) + 1,
          (long)id_act,
          (game_get_player(game) ? player_get_health(game_get_player(game)) : 0),
          (game_get_player(game) ? player_get_max_health(game_get_player(game)) : 0),
          (game_get_player(game) ? player_get_damage(game_get_player(game)) : 0));
  screen_area_puts(ge->descript, str);
  ...
  for (i = 0; i < MAX_OBJECTS; i++)
  {
    ...
    if (temp_obj)
    {
      loc = game_get_object_location(game, object_get_id(temp_obj));
      if (loc == WITH_PLAYER)
      {
        sprintf(str, "      %s", object_get_name(temp_obj));
        screen_area_puts(ge->descript, str);
  ...
  screen_area_puts(ge->help, " move/m  take/t  drop/d  chat/c  attack/a  inspect/i  exit/e  abandon/ab  recruit/r  colab/co  save/s  load/l ");

  last_cmd = command_get_code(game_get_last_command(game));

  if (last_cmd == CHAT || last_cmd == ATTACK || last_cmd == INSPECT || last_cmd == MOVE || last_cmd == RECRUIT || last_cmd == ABANDON || last_cmd == USE || last_cmd == LOAD || last_cmd == SAVE || last_cmd == COLAB|| last_cmd ==TAKE || last_cmd==DROP)
  {
    msg = game_get_s(game);
    if (msg != NULL && msg[0] != '\0')
    {
  ...
  if (game_get_event(game) == TRUE)
  {
    sprintf(str, " %s", game_get_e(game));
    screen_area_puts(ge->feedback, str);
    game_set_event(game, FALSE);
  }
