...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game)
{
  ...
  screen_area_puts(ge->help, " move/m  take/t  drop/d  chat/c  attack/a  inspect/i  exit/e  abandon/ab recruit/r  save/s  load/l ");

  last_cmd = command_get_code(game_get_last_command(game));

  if (last_cmd == CHAT || last_cmd == ATTACK || last_cmd == INSPECT || last_cmd == MOVE || last_cmd == RECRUIT || last_cmd == ABANDON || last_cmd == USE || last_cmd == LOAD || last_cmd == SAVE)
  {
    msg = game_get_s(game);
    if (msg != NULL && msg[0] != '\0')
    {
