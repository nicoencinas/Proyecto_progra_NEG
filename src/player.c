...
Player *player_create(Id id)
{
    ...
    new_player->damage = 5;
    new_player->dtaken = 5;
    new_player->max_health = 30;
    new_player->n_turn = 1;
    new_player->max_turn = 1;
    new_player->teammate = NO_ID;
...
Status player_set_teammate(Player *player, Id teammate)
{
    if (!player)
        return ERROR;
    if (player->teammate != NO_ID ) /* Solo se puede tener un compañero como mucho */
    {
        return ERROR;
    }
