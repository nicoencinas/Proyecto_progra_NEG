...
Status game_create_from_file(Game *game, char *filename)
{
  ...
  if (game_reader_load_followers(game, filename) == ERROR)
    return ERROR;
  if (game_reader_load_multiplayer(game, filename) == ERROR)
    return ERROR;
...
Status game_reader_load_players(Game *game, char *filename)
{
  ...
  while (fgets(line, WORD_SIZE, file))
  {
    if (strncmp("#p:", line, 3) == 0)
    {
      ...
      player_set_max_inv(p, max_inv);
      player_set_teammate(p, teammate_id);
...
Status game_reader_load_multiplayer(Game *game, char *filename)
{
  FILE *file = NULL;
  char line[WORD_SIZE];
  int multi = 0;
  ...
  while (fgets(line, sizeof(line), file)) {
    if (strncmp(line, "#m:", 3) == 0) {
      if (sscanf(line, "#m:%d", &multi) != EOF) {
        game_set_multiplayer(game, (multi == 1) ? TRUE : FALSE);
      }
    }
  }
...
Status game_management_save(Game *game, char *filename)
{
  ...
  for (i = 0; i < game_get_n_players(game); i++)
  {
    ...
    fprintf(file, "#p:%ld|%s|%s|%ld|%d|%d|%ld\n",
            player_get_id(p),
            player_get_name(p),
            player_get_gdesc(p),
            player_get_location(p),
            player_get_health(p),
            player_get_max_inv(p),
            player_get_teammate(p));
  }
