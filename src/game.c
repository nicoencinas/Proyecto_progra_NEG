...
char *data_get_e(InterfaceData *data)
{
  if (!data)
    return NULL;
  return data->e;
...
Status data_set_e(InterfaceData *data, char *m)
{
  if (!data || !m)
    return ERROR;
  strcpy(data->e, m);
...
char *game_get_e(Game *game)
{
  ...
  return data_get_e(game->Data[game->turn]);
}
...
Status game_set_e(Game *game, char *message)
{
  ...
  return data_set_e(game->Data[game->turn], message);
}
...
{
  if (!game || index < 0 || index >= game->n_spaces)
    return NULL;
...
{
  if (!game || index < 0 || index >= game->n_players)
    return NULL;
