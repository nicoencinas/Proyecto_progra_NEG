...
Status use_execute(Game *game, Object *obj)
{
  ...
  if (strcasecmp(name, "W.Shield") == 0 || strcasecmp(name, "S.Shield") == 0)
    return use_sword(game, obj);
  ...
  if (strcasecmp(name, "Flag") == 0 || strcasecmp(name, "Flag") == 0)
    return use_flag(game, obj);
...
Status use_flag(Game *game, Object *obj)
{
  Player *pl;
  if (!game || !obj)
    return ERROR;
