...
struct _Space
{
  ...
  Set *ch;
  char gdesc[GDESC_LINES][MAX_CHARACTERS];
  Bool discovered;
};
...
{
  if (!space || line < 0 || line >= GDESC_LINES)
  {
    return NULL;
  }
...
{
  if (!space || !str || line < 0 || line >= GDESC_LINES)
  {
    return ERROR;
  }
